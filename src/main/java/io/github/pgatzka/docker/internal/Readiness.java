...
public final class Readiness {
    ...
    public static final class NotReadyException extends RuntimeException {
        public NotReadyException(String msg) { super(msg); }
    }
    ...
    public static void logLine(DockerClient c, String containerId, String regex,
                               Duration timeout) {
        ...
        try {
            c.logContainerCmd(containerId).withStdOut(true).withStdErr(true)
                .withFollowStream(true).withTailAll().exec(callback)
        ...
        if (!matched.get()) {
            throw new NotReadyException(
                "Log line matching /" + regex + "/ not found within " + timeout);
        }
    }
